    ...
    public ArrayList<Article> getListArticleSortByDate() {
        ...
        try(Connection con = DBConnect.getDBConnection()) {
            PreparedStatement ps = (PreparedStatement) con
                    .prepareStatement("select * from article ORDER BY published_date DESC");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                ls.add(new Article( rs.getInt("id"), rs.getString("title"), rs.getString("content"), rs.getString("image"),rs.getDate("published_date") ) );
            }
